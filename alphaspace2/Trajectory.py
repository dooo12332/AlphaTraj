...
class Trajectory(list):
    ...
    def gen_dpockets(self, clust_distance, bin_size=None):
        ...
        if bin_size is None:
            self.beta_dpocket_labels = fcluster(linkage(beta_coords, method='average'), clust_distance,
                                           criterion='distance') - 1
        else:
            self.beta_dpocket_labels = _binCluster(coords=beta_coords, distance=clust_distance , bin_size=bin_size)
        self.dpocket_beta_labels = _group(self.beta_dpocket_labels)
