...
def _pre_process_pdbqt(traj, truncation_length=0,frame:int=0):
    ...
    don_type = []
    acc_type = []
    prot_coord = traj.xyz[frame] * 10
    prot_tree = spatial.cKDTree(prot_coord)
    prot_types = []
    #print(f'traj.adv_atom_types.len={len(traj.adv_atom_types)}')
    #print(f'traj.top.atoms.len={len(list(traj.top.atoms))}')
