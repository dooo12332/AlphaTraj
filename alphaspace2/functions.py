...
def _getNonpolarRatio(protein_snapshot, alpha_lining_idx,frame:int=0):
    areas = getSASA(protein_snapshot=protein_snapshot,frame=frame)
...
def getSASA(protein_snapshot, cover_atom_coords=None,frame:int=0):
    ...
    if cover_atom_coords is None:
        #print(f'protein_snapshot.xyz.shape={protein_snapshot.xyz.shape}')
        ...
        xyz = np.array(np.expand_dims(np.concatenate((protein_snapshot.xyz[0], cover_atom_coords), axis=0), axis=0),
                       dtype=np.float32)
        atom_radii = [_ATOMIC_RADII[atom.element.symbol] for atom in protein_snapshot.topology.atoms] + [0.17 for _ in range(xyz.shape[1])]
...
def _markInRange(query_points:np.ndarray, ref_points:np.ndarray, cutoff):
    # indices=[]
    # c2=np.power(cutoff,2)
    # for i in range(query_points.shape[0]):
    #     for j in range(ref_points.shape[0]):
    #         if np.sum(np.power(ref_points[j]-query_points[i],2))>c2:
    #             indices.append(i)
    #             break
